...
import { AcceptInviteForm } from "../../accept-invite/accept-invite-form";
...
export default async function InviteAcceptPage({
  ...
  return (
    <main className="min-h-screen w-full flex">
      <div
        className="hidden lg:flex flex-col justify-around w-1/2 px-12 py-10"
        style={{ background: "linear-gradient(135deg, #0f0f1a 0%, #09090f 50%, #1e0e06 100%)" }}
      >
        <div className="flex items-center gap-2.5">
          <Image src="/logo.svg" alt="NaijaShield logo" width={180} height={56} className="h-8 w-auto" priority />
        ...
        <div className="space-y-6">
          <div className="relative w-48 h-48">
            ...
            <div className="absolute top-[30%] left-[60%] w-2 h-2 rounded-full animate-ping" style={{ background: "#ef4444" }} />
            <div className="absolute top-[55%] left-[35%] w-1.5 h-1.5 rounded-full animate-ping" style={{ background: "#f59e0b", animationDelay: "0.5s" }} />
            <div className="absolute top-[70%] left-[65%] w-1 h-1 rounded-full animate-ping" style={{ background: "#e8581a", animationDelay: "1s" }} />
          </div>
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm font-light leading-relaxed" style={{ color: "#9c7c6e" }}>
              Set your password to activate your access to the fraud intelligence operations dashboard.
            </p>
          </div>

          <div className="grid grid-cols-3 gap-4 pt-4" style={{ borderTop: "1px solid #1a1a2e" }}>
            {[
              ...
              <div key={stat.label}>
                <p className="text-xl font-bold" style={{ color: "#e8581a" }}>{stat.value}</p>
                <p className="text-xs font-light mt-0.5" style={{ color: "#9c7c6e" }}>{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
